...
# Étienne Deparis   <etienne@depar.is>            # Fossil and Ruby virtual env support
...
LP_RUBY_VENV_CMD=
if [[ "$LP_ENABLE_RUBY_VENV" = 1 ]] ; then
    if command -v rvm-prompt >/dev/null ; then
        LP_RUBY_VENV_CMD=_lp_rvm_venv
    elif command -v rbenv >/dev/null ; then
        LP_RUBY_VENV_CMD="rbenv version-name"
    else
        LP_ENABLE_RUBY_VENV=0
    fi
fi
_lp_rvm_venv() {
    rvm-prompt | sed -n '/@/{s/\([^-]*\)-\([0-9.]*\)\?\([^@]*\)\?@\(.*\)/\1 \2 (\4)/p;q};s/\([^-]*\)-\([0-9.]*\)\?\(.*\)\?/\1 \2/p'
}
...
_lp_set_prompt()
{
    ...
    # Display the current Ruby environnement, if available.
    LP_RUBY_VENV=
    if [[ "$LP_ENABLE_RUBY_VENV" = 1 ]] ; then
        LP_RUBY_VENV=$(eval ${LP_RUBY_VENV_CMD})

        if [ -n "$LP_RUBY_VENV" ] ; then
            LP_RUBY_VENV=" [${LP_COLOR_RUBYENV}${LP_RUBY_VENV}${NO_COL}]"
        fi
    fi
    ...
    if [[ -z "$LP_PS1" ]] ; then
        ...
        PS1="${PS1}${LP_PWD}${LP_BRACKET_CLOSE}${LP_VENV}${LP_RUBY_VENV}${LP_PROXY}"
